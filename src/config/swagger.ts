      ...
      schemas: {
        ...
        ReservationDetailResponse: {
          ...
          properties: {
            ...
            shopName: { type: 'string', example: '네일내일 강남점' },
            shopPhoneNumber: { type: 'string', nullable: true, example: '02-1234-5678' },
            shopRating: { type: 'number', format: 'double', example: 4.8 },
            shopReviewCount: { type: 'integer', example: 24 },
            address: { type: 'string', example: '서울시 강남구 테헤란로 1 2층' },
            addressDetail: { type: 'string', nullable: true, example: '2층' },
            latitude: { type: 'number', format: 'double', example: 37.4979 },
            longitude: { type: 'number', format: 'double', example: 127.0276 },
            shopThumbnailUrl: { type: 'string', format: 'uri', nullable: true },
            shopBusinessHours: {
              nullable: true,
              description: '요일별 영업시간 JSON. 데이터가 없으면 null',
            },
            ...
            distanceMeters: {
              type: 'integer',
              nullable: true,
              description: '현재 위도와 경도를 함께 전달한 경우에만 계산',
              example: 850,
            },
            reservedAt: { type: 'string', format: 'date-time' },
            basePrice: { type: 'integer', nullable: true, example: 40000 },
            removalPrice: { type: 'integer', nullable: true, example: 5000 },
            designExtraPrice: { type: 'integer', nullable: true, example: 10000 },
            optionExtraPrice: { type: 'integer', nullable: true, example: 0 },
            couponDiscount: { type: 'integer', example: 0 },
            totalPrice: { type: 'integer', nullable: true, example: 55000 },
            shopComment: { type: 'string', nullable: true },
            nailType: { type: 'string', enum: ['HAND', 'PEDICURE', 'BOTH'] },
            removalTypes: {
              type: 'array',
              items: { type: 'string', enum: ['EXTENSION', 'PARTS', 'BASIC', 'NONE'] },
            },
            images: { type: 'array', items: { type: 'string', format: 'uri' } },
            status: { type: 'string', enum: ['CONFIRMED', 'COMPLETED', 'CANCELLED'] },
            designName: { type: 'string', nullable: true, example: '여름 자개 네일' },
          },
        },
